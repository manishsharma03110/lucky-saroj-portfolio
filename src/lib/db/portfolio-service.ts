...
export async function createPortfolioProject(i:ProjectMutationInput):Promise<string>{
  ...
  try{
    return await withCmsTransaction(async tx=>{
      ...
      await tx.db.insert(sql`INSERT INTO portfolio_projects(id,title,slug,client_name,year,category_id,description,challenge,approach,result,thumbnail_url,thumbnail_alt,video_url,is_featured,status,seo_title,seo_description,og_title,og_description,og_image_url,twitter_title,twitter_description,twitter_image_url,revision) VALUES (${id},${i.title},${i.slug},${i.clientName||null},${i.year??null},${i.categoryId||null},${i.description||null},${i.challenge||null},${i.approach||null},${i.result||null},${thumbnail.url},${alt},${video.url},${i.isFeatured??false},${i.status},${i.seoTitle||null},${i.seoDescription||null},${i.ogTitle||null},${i.ogDescription||null},${i.ogImageUrl||null},${i.twitterTitle||null},${i.twitterDescription||null},${i.twitterImageUrl||null},1)`);
...
export async function updatePortfolioProject(id:string,expectedRevision:number,i:ProjectMutationInput,testSynchronization?:MutationTestSynchronization):Promise<number>{
  ...
  try{
    return await withCmsTransaction(async tx=>{
      ...
      const u=await tx.db.update<{revision:number}>(sql`UPDATE portfolio_projects SET title=${i.title},slug=${i.slug},client_name=${i.clientName||null},year=${i.year??null},category_id=${i.categoryId||null},description=${i.description||null},challenge=${i.challenge||null},approach=${i.approach||null},result=${i.result||null},thumbnail_url=${thumbnail.url},thumbnail_alt=${alt},video_url=${video.url},is_featured=${i.isFeatured??false},status=${i.status},seo_title=${i.seoTitle||null},seo_description=${i.seoDescription||null},og_title=${i.ogTitle||null},og_description=${i.ogDescription||null},og_image_url=${i.ogImageUrl||null},twitter_title=${i.twitterTitle||null},twitter_description=${i.twitterDescription||null},twitter_image_url=${i.twitterImageUrl||null},updated_at=now(),revision=revision+1 WHERE id=${id} AND revision=${expectedRevision} RETURNING revision`);
