...
const optionalHostedVideoReference = z.string().trim().max(2048).optional().or(z.literal("")).refine((v) => !v || isDirectVideoUrl(v), "Enter a direct MP4/WebM/video file URL or path.");
const optionalFrameCopy = z.string().trim().max(160).optional().or(z.literal(""));
...
export const projectSchema = z.object({
  ...
  hostedVideoUrl: optionalHostedVideoReference,
  videoUrl: optionalYouTubeReference, videoAssetId: optionalAssetId,
  videoEditorName: optionalFrameCopy, videoEditorRole: optionalFrameCopy, videoTagline: optionalFrameCopy, videoBottomLabel: optionalFrameCopy,
