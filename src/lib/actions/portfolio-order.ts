...
export async function reorderPortfolioProjects(ids:string[]):Promise<void>{
  ...
  await withCmsTransaction(async(tx)=>{
    const all=await tx.db.select<{id:string}>(sql`SELECT id FROM portfolio_projects`);
    const existing=new Set(all.rows.map(row=>row.id));
    if(ids.length!==existing.size||ids.some(id=>!existing.has(id)))throw new Error("Project list changed. Reload and try again.");
  ...
  revalidatePath("/admin/portfolio");revalidatePath("/portfolio");revalidatePath("/");
}
