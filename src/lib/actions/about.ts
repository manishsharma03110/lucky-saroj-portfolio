...
export async function updateAboutProfile(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    await replaceAbout({
      ...
      profileImageAssetId: data.profileImageAssetId || null,
      yearsExperience: data.yearsExperience,
      projectsCompleted: data.projectsCompleted,
      clientCount: data.clientCount,
      viewsGenerated: data.viewsGenerated,
      skills: skills.map((name, displayOrder) => ({ name, displayOrder })),
      tools: tools.map((name, displayOrder) => ({ name, displayOrder })),
    });
  } catch (error) {
