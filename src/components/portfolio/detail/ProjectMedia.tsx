...
export function ProjectGallery({ project, media, copy = {} }: { project: Project; media: Media[]; copy?: MediaCopy }) {
  ...
  const mediaAlt = `${project.title} ${copy.mediaAltSuffix || "project media"}`;
  return <section className="border-t border-white/10 bg-[var(--background-primary)] py-12 sm:py-16 lg:py-20 2xl:py-24"><div className="mx-auto w-full max-w-[1560px] px-5 sm:px-8 lg:px-12 2xl:px-16"><p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--accent-primary)]">{copy.mediaEyebrow || "Additional media"}</p><h2 className="mt-4 font-display text-3xl font-semibold tracking-[-0.04em] text-[var(--text-primary)] sm:text-4xl lg:text-5xl">{copy.mediaHeading || "More from the project."}</h2><div className="mt-8 grid gap-5 sm:gap-6 md:grid-cols-2 lg:mt-10 lg:gap-8">{items.map((item, index) => <div key={item.id} className={`relative aspect-video overflow-hidden rounded-[10px] border border-white/10 bg-[var(--surface-primary)] ${index % 3 === 0 ? "md:col-span-2" : ""}`}>{item.type === "video" ? <VideoPlayer videoUrl={item.url} title={mediaAlt} className="h-full w-full" /> : <div className="h-full w-full bg-cover bg-center" style={{ backgroundImage: `url('${item.url}')` }} role="img" aria-label={mediaAlt} />}</div>)}</div></div></section>;
}
