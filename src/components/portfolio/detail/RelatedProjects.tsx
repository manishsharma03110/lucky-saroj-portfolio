            ...
            return (
              <article key={project.id} className="group min-w-0">
                <Link href={`/portfolio/${project.slug}`} className="block rounded-[10px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)]">
                  <div className="relative aspect-video overflow-hidden rounded-[10px] border border-white/10 bg-[var(--surface-primary)]">
                    {imageUrl ? (
                      canUseOptimizedImage(imageUrl) ? <Image src={imageUrl} alt={alt} fill sizes="(max-width: 767px) 100vw, 33vw" className="object-contain transition-transform duration-500 motion-reduce:transition-none group-hover:scale-[1.01]" /> : <div role="img" aria-label={alt} className="absolute inset-0 bg-contain bg-center bg-no-repeat transition-transform duration-500 motion-reduce:transition-none group-hover:scale-[1.01]" style={{ backgroundImage: `url('${imageUrl}')` }} />
                    ) : (
