...
export function ProjectCard({ project, categoryName, mediaUrl, layout = "standard" }: { project: Project; categoryName?: string; mediaUrl?: string | null; layout?: Layout }) {
  ...
  if (featured) {
    return (
      <article className="border-b border-white/10 pb-12 sm:pb-14">
        <Link href={`/portfolio/${project.slug}`} className="group grid rounded-[12px] gap-8 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-4 focus-visible:ring-offset-[var(--background-primary)] lg:grid-cols-[minmax(0,1.5fr)_minmax(20rem,0.68fr)] lg:items-center lg:gap-12 2xl:gap-16">
  ...
  return (
    <article>
      <Link href={`/portfolio/${project.slug}`} className="group block rounded-[12px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-4 focus-visible:ring-offset-[var(--background-primary)]">
