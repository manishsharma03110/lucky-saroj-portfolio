...
function ProjectVisual({ project, categoryName, mediaUrl, layout, copy }: { project: Project; categoryName?: string; mediaUrl?: string | null; layout: Layout; copy: CardCopy }) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-[12px] border border-white/10 bg-[var(--surface-primary)] shadow-[0_22px_70px_rgba(0,0,0,0.2)] transition-[border-color,box-shadow] duration-500 motion-reduce:transition-none group-hover:border-[var(--accent-border)] group-hover:shadow-[0_28px_90px_rgba(0,0,0,0.34)] ${aspect}`}>
      {visualUrl ? <div className="absolute inset-0 bg-cover bg-center transition-transform duration-500 motion-reduce:transition-none group-hover:scale-[1.025]" style={{ backgroundImage: `url('${visualUrl}')` }} role="img" aria-label={`${project.title} ${copy.previewAltSuffix}`} /> : (
...
export function ProjectCard({ project, categoryName, mediaUrl, layout = "standard", copy }: { project: Project; categoryName?: string; mediaUrl?: string | null; layout?: Layout; copy: CardCopy }) {
  ...
  if (featured) return <article className="border-b border-white/10 pb-12 sm:pb-14"><Link href={`/portfolio/${project.slug}`} className="group grid rounded-[12px] gap-8 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-4 focus-visible:ring-offset-[var(--background-primary)] lg:grid-cols-[minmax(0,1.5fr)_minmax(20rem,0.68fr)] lg:items-center lg:gap-12 2xl:gap-16"><ProjectVisual project={project} categoryName={categoryName} mediaUrl={mediaUrl} layout="featured" copy={copy} /><div className="min-w-0 lg:pr-4"><p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--accent-primary)]">{copy.featuredLabel}</p><h2 className="mt-4 break-words font-display text-[clamp(2.25rem,3.6vw,3.5rem)] font-semibold leading-none tracking-[-0.045em] text-[var(--text-primary)]">{project.title}</h2>{metadata && <p className="mt-4 break-words text-sm text-[var(--text-muted)]">{metadata}</p>}{project.description && <p className="mt-6 line-clamp-4 text-base leading-7 text-[var(--text-secondary)]">{project.description}</p>}<span className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-[var(--text-primary)] transition-colors group-hover:text-[var(--accent-hover)]">{copy.viewLabel} <ArrowUpRight size={17} className="transition-transform duration-300 motion-reduce:transition-none group-hover:translate-x-0.5 group-hover:-translate-y-0.5" aria-hidden /></span></div></Link></article>;

  return <article><Link href={`/portfolio/${project.slug}`} className="group block rounded-[12px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-4 focus-visible:ring-offset-[var(--background-primary)]"><ProjectVisual project={project} categoryName={categoryName} mediaUrl={mediaUrl} layout={layout} copy={copy} /><div className="flex min-w-0 items-start justify-between gap-5 border-b border-white/10 py-5 sm:py-6"><div className="min-w-0"><h2 className="break-words font-display text-2xl font-semibold tracking-[-0.035em] text-[var(--text-primary)] sm:text-3xl">{project.title}</h2>{metadata && <p className="mt-2 break-words text-sm text-[var(--text-muted)]">{metadata}</p>}</div><ArrowUpRight size={20} className="mt-1 shrink-0 text-[var(--accent-primary)] transition-transform duration-300 motion-reduce:transition-none group-hover:translate-x-0.5 group-hover:-translate-y-0.5" aria-hidden /></div></Link></article>;
}
