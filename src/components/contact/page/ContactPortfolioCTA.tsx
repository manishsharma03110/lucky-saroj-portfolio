...
export function ContactPortfolioCTA({ heading = "Have a project to discuss?", description = "Explore the work and see how different stories have been shaped.", label = "View My Work", url = "/portfolio" }: { heading?: string; description?: string; label?: string; url?: string }) {
      ...
      <div className="mx-auto w-full max-w-[1280px] px-5 sm:px-8 lg:px-12">
        <div className="group relative grid overflow-hidden rounded-lg border border-[var(--accent-primary)]/30 bg-[var(--surface-primary)] p-6 transition-[border-color,box-shadow] duration-500 hover:border-[var(--accent-primary)]/50 hover:shadow-[0_0_38px_rgba(59,130,246,0.08)] sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center sm:gap-7 sm:p-8 motion-reduce:transition-none">
          ...
          <div className="relative mt-5 min-w-0 sm:mt-0">
            <h2 id="contact-portfolio-title" className="font-display text-2xl font-semibold tracking-[-0.035em] text-[var(--text-primary)]">{heading}</h2>
            <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">{description}</p>
          </div>
          <Link href={url} className="relative mt-6 inline-flex min-h-12 items-center justify-center gap-3 rounded-md border border-[var(--accent-primary)]/55 px-6 py-3 text-sm font-semibold text-[var(--accent-hover)] transition-[background-color,border-color] duration-300 hover:border-[var(--accent-hover)] hover:bg-[var(--accent-primary)]/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] sm:mt-0 motion-reduce:transition-none">{label} <ArrowRight size={17} className="transition-transform duration-300 group-hover:translate-x-1 motion-reduce:transition-none" aria-hidden /></Link>
        </div>
      </div>
    </motion.section>
  );
