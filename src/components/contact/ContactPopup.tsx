...
export function ContactPopup({ copy, optionsConfig }: { copy: Record<string, string>; optionsConfig?: string }) {
  ...
  return (
    <AnimatePresence>
      {open && (
        <motion.div initial={reduceMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reduceMotion ? 0 : 0.24 }} className="fixed inset-0 z-[100] flex min-h-[100dvh] items-center justify-center overflow-hidden bg-[var(--contact-scrim)] p-3 backdrop-blur-md sm:p-4">
          ...
          <motion.div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="contact-popup-title" aria-describedby="contact-popup-description" initial={reduceMotion ? false : { opacity: 0, y: 18, scale: 0.985 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.99 }} transition={{ duration: reduceMotion ? 0 : 0.26, ease: [0.22, 1, 0.36, 1] }} className="relative max-h-[92dvh] w-[calc(100vw-24px)] max-w-[500px] overflow-x-hidden overflow-y-auto rounded-[18px] border border-white/15 bg-[var(--background-primary)] text-[var(--text-primary)] shadow-[var(--contact-shadow)] [scrollbar-color:var(--border-primary)_transparent] [scrollbar-width:thin] sm:max-h-[87dvh] sm:w-[calc(100vw-32px)] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/15 hover:[&::-webkit-scrollbar-thumb]:bg-white/20" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="relative min-h-[218px] overflow-hidden bg-[var(--surface-primary)] sm:min-h-[238px] [@media(max-height:800px)]:sm:min-h-[205px] [@media(max-height:700px)]:sm:min-h-[188px]">
              ...
              <div className="absolute inset-0 bg-[linear-gradient(to_top,var(--background-primary)_0%,rgba(var(--background-rgb),0.96)_12%,rgba(var(--background-rgb),0.72)_25%,rgba(var(--background-rgb),0.18)_58%,rgba(var(--background-rgb),0.14)_100%)]" aria-hidden="true" />
              <div className="absolute inset-x-0 bottom-0 z-10 px-6 pb-4 sm:px-7 sm:pb-5 [@media(max-height:800px)]:sm:pb-3.5">
                <p className="text-[0.6rem] font-semibold uppercase tracking-[0.28em] text-[var(--text-secondary)]">{copy.popupEyebrow}</p>
                <h2 id="contact-popup-title" className="mt-1.5 max-w-[12ch] font-display text-[clamp(2.25rem,10vw,2.75rem)] font-bold uppercase leading-[0.84] tracking-[0] text-[var(--text-primary)] [@media(max-height:700px)]:sm:text-[2.25rem]">{copy.popupHeading}<span className="relative mt-1 block w-max max-w-full -rotate-2 font-display text-[0.62em] font-medium italic normal-case leading-[0.92] text-[var(--accent-primary)] after:absolute after:-bottom-1 after:left-2 after:h-px after:w-[88%] after:-rotate-2 after:bg-[var(--accent-primary)]">{copy.popupHeadingAccent}</span></h2>
                <p id="contact-popup-description" className="mt-3 max-w-[25rem] text-[0.72rem] leading-[1.45] text-[var(--text-secondary)] [@media(max-height:700px)]:sm:mt-2 [@media(max-height:700px)]:sm:text-[0.68rem]">{copy.popupDescription}</p>
              </div>
            </div>
