      ...
      {open && (
        <motion.div initial={reduceMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reduceMotion ? 0 : 0.24 }} className="contact-popup-backdrop fixed inset-0 z-[100] flex min-h-[100dvh] items-center justify-center overflow-hidden bg-[var(--contact-scrim)] p-3 backdrop-blur-md sm:p-4">
          ...
          <motion.div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="contact-popup-title" aria-describedby="contact-popup-description" initial={reduceMotion ? false : { opacity: 0, y: 18, scale: 0.985 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.99 }} transition={{ duration: reduceMotion ? 0 : 0.26, ease: [0.22, 1, 0.36, 1] }} className="relative max-h-[92dvh] w-[calc(100vw-24px)] max-w-[500px] overflow-x-hidden overflow-y-auto rounded-[18px] border border-white/15 bg-[var(--background-primary)] text-[var(--text-primary)] shadow-[var(--contact-shadow)] [scrollbar-color:var(--border-primary)_transparent] [scrollbar-width:thin] sm:max-h-[87dvh] sm:w-[calc(100vw-32px)] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/15 hover:[&::-webkit-scrollbar-thumb]:bg-white/20" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="contact-popup-hero relative min-h-[218px] overflow-hidden bg-[var(--surface-primary)] sm:min-h-[238px] [@media(max-height:800px)]:sm:min-h-[205px] [@media(max-height:700px)]:sm:min-h-[188px]">
