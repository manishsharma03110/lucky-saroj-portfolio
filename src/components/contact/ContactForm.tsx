...
export function ContactForm({ projectCategories = [] }: { projectCategories?: string[] }) {
  ...
  return (
    <form action={formAction} className="space-y-5" aria-label="Project inquiry form" aria-busy={pending}>
      ...
      <p className="text-xs leading-5 text-[var(--text-muted)]"><span className="text-[var(--accent-primary)]" aria-hidden>*</span> Required fields</p>
      <div className="grid gap-5 md:grid-cols-2">
        <div>
          <label htmlFor="name" className={labelClasses}>Your Name <RequiredMark /></label>
          <Input id="name" name="name" autoComplete="name" placeholder="Your name" required maxLength={120} disabled={pending} className={`${fieldClasses} ${state.fieldErrors?.name ? "!border-red-400/60" : ""}`} aria-invalid={Boolean(state.fieldErrors?.name)} aria-describedby={state.fieldErrors?.name ? "name-error" : undefined} />
        ...
        <div>
          ...
          <Input id="email" name="email" type="email" autoComplete="email" placeholder="you@example.com" required disabled={pending} className={`${fieldClasses} ${state.fieldErrors?.email ? "!border-red-400/60" : ""}`} aria-invalid={Boolean(state.fieldErrors?.email)} aria-describedby={state.fieldErrors?.email ? "email-error" : undefined} />
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div>
          ...
          <Input id="phone" name="phone" type="tel" autoComplete="tel" placeholder="Optional" maxLength={20} disabled={pending} className={`${fieldClasses} ${state.fieldErrors?.phone ? "!border-red-400/60" : ""}`} aria-invalid={Boolean(state.fieldErrors?.phone)} aria-describedby={state.fieldErrors?.phone ? "phone-error" : undefined} />
        ...
        <div>
          ...
          <ContactSelect id="budgetRange" name="budgetRange" placeholder="Select budget" options={BUDGET_RANGES} required disabled={pending} invalid={Boolean(state.fieldErrors?.budgetRange)} describedBy={state.fieldErrors?.budgetRange ? "budget-error" : undefined} />
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div>
          ...
          <ContactSelect id="projectType" name="projectType" placeholder="Select category" options={categoryOptions} required disabled={pending} invalid={Boolean(state.fieldErrors?.projectType)} describedBy={state.fieldErrors?.projectType ? "category-error" : undefined} />
        ...
        <div>
          ...
          <ContactSelect id="videoType" name="videoType" placeholder="Select video type" options={VIDEO_TYPES} required disabled={pending} invalid={Boolean(state.fieldErrors?.videoType)} describedBy={state.fieldErrors?.videoType ? "video-type-error" : undefined} />
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div>
          ...
          <ContactSelect id="projectTimeline" name="projectTimeline" placeholder="Select desired timeline" options={PROJECT_TIMELINES} disabled={pending} invalid={Boolean(state.fieldErrors?.projectTimeline)} describedBy={state.fieldErrors?.projectTimeline ? "timeline-error" : undefined} />
        ...
        <div>
          ...
          <Input id="referenceUrl" name="referenceUrl" type="url" inputMode="url" autoComplete="url" placeholder="https://" maxLength={500} disabled={pending} className={`${fieldClasses} ${state.fieldErrors?.referenceUrl ? "!border-red-400/60" : ""}`} aria-invalid={Boolean(state.fieldErrors?.referenceUrl)} aria-describedby={state.fieldErrors?.referenceUrl ? "reference-error" : undefined} />
      ...
      <div>
        ...
        <Textarea id="message" name="message" rows={6} placeholder="Share the goals, audience, style, and any specific requirements." required maxLength={4000} disabled={pending} className={`${fieldClasses} min-h-40 resize-y py-4 ${state.fieldErrors?.message ? "!border-red-400/60" : ""}`} aria-invalid={Boolean(state.fieldErrors?.message)} aria-describedby={`message-helper${state.fieldErrors?.message ? " message-error" : ""}`} />
