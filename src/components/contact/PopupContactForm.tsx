...
export function PopupContactForm() {
  ...
  return (
    <form action={formAction} className="relative -mt-px space-y-2 bg-[linear-gradient(to_bottom,var(--background-primary)_0%,var(--background-primary)_4rem)] px-5 pb-4 pt-2 sm:px-7 sm:pb-5 [@media(max-height:800px)]:sm:space-y-1.5 [@media(max-height:800px)]:sm:pb-3" aria-label="Project inquiry form" aria-busy={pending}>
      ...
      <div><label htmlFor="popup-project-type" className="sr-only">Project Type</label><PopupContactSelect id="popup-project-type" name="projectType" placeholder="Project Type" options={POPUP_PROJECT_TYPES} onValueChange={setProjectType} invalid={Boolean(state.fieldErrors?.projectType)} describedBy={state.fieldErrors?.projectType ? "popup-project-type-error" : undefined} />{state.fieldErrors?.projectType && <p id="popup-project-type-error" className={errorClass}>{state.fieldErrors.projectType}</p>}</div>
      ...
      <div><label htmlFor="popup-message" className="sr-only">Your Message</label><div className="relative"><SquarePen size={17} className="pointer-events-none absolute left-4 top-3.5 z-10 text-[var(--accent-primary)]" aria-hidden="true" /><textarea id="popup-message" name="message" rows={4} placeholder="Your Message" required maxLength={4000} aria-invalid={Boolean(state.fieldErrors?.message)} aria-describedby={state.fieldErrors?.message ? "popup-message-error" : undefined} className={`${fieldClass} min-h-[5.5rem] resize-y py-3 [@media(max-height:800px)]:sm:min-h-[5.125rem] ${state.fieldErrors?.message ? "border-red-400/60 hover:border-red-400/75" : ""}`} /></div>{state.fieldErrors?.message && <p id="popup-message-error" className={errorClass}>{state.fieldErrors.message}</p>}</div>
