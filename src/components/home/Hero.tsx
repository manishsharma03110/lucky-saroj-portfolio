  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-white/10 bg-[var(--background-primary)]">
      <div className="absolute inset-0 -z-20 overflow-hidden bg-[var(--surface-primary)]" aria-hidden={heroImageAlt ? undefined : "true"}>
        {optimizedVisual ? (
          <Image src={visualUrl} alt={heroImageAlt} fill priority sizes="100vw" className="scale-[1.01] object-cover object-[70%_center] motion-safe:transition-transform motion-safe:duration-[1600ms] sm:object-center lg:object-[60%_center]" />
        ) : (
          <div className="h-full w-full scale-[1.01] bg-cover bg-[70%_center] motion-safe:transition-transform motion-safe:duration-[1600ms] sm:bg-center lg:bg-[60%_center]" style={{ backgroundImage: `url('${visualUrl}')` }} role={heroImageAlt ? "img" : undefined} aria-label={heroImageAlt || undefined} />
        )}
      </div>
      ...
      <div className="mx-auto flex min-h-[570px] w-full max-w-[1560px] items-end px-5 pb-12 pt-24 sm:min-h-[640px] sm:px-8 sm:pb-16 sm:pt-28 md:min-h-[680px] lg:min-h-[720px] lg:items-center lg:px-12 lg:py-24 xl:min-h-[760px] 2xl:px-16">
        <div className="max-w-[760px] 2xl:max-w-[860px]">
          ...
          <div className="mt-7 flex flex-col items-start gap-3 sm:mt-9 sm:flex-row">
            <Button href={primaryUrl} variant="cine-solid" withArrow className="min-h-12 !rounded-md !bg-[var(--accent-primary)] !px-6 !py-3 !font-semibold !text-[var(--background-primary)] hover:!bg-[var(--accent-hover)] sm:px-7">{primaryLabel}</Button>
            {hasShowreel && (
              <Button href={showreelUrl} variant="cine-outline" className="min-h-12 gap-2 !rounded-md !border-white/25 !px-6 !py-3 !text-[var(--text-primary)] hover:!border-[var(--accent-primary)] hover:!text-[var(--accent-hover)] sm:px-7">
