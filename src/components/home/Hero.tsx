...
export function Hero({ heading, subheading, description, heroImageUrl, heroImageAlt, primaryLabel, primaryUrl, showreelLabel, showreelUrl, hasShowreel }: { heading:string; subheading:string; description:string; heroImageUrl?:string|null; heroImageAlt:string; primaryLabel:string; primaryUrl:string; showreelLabel:string; showreelUrl:string; hasShowreel:boolean; }) {
  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-white/10 bg-[var(--background-primary)]">
      <div className="absolute inset-0 -z-20 origin-center overflow-hidden bg-[var(--surface-primary)]" aria-hidden={heroImageAlt ? undefined : "true"}>
        <HeroParallax><Image src={visualUrl} alt={heroImageAlt} fill priority fetchPriority="high" quality={40} sizes="100vw" className={`${motionStyles.heroVisual} object-cover object-[68%_center] min-[480px]:object-[64%_center] sm:object-center lg:object-[60%_center]`} /></HeroParallax>
      </div>
      <div className="pointer-events-none absolute inset-0 -z-10 bg-[linear-gradient(90deg,rgba(var(--background-rgb),0.98)_0%,rgba(var(--background-rgb),0.88)_38%,rgba(var(--background-rgb),0.38)_70%,rgba(var(--background-rgb),0.22)_100%)] max-lg:bg-[linear-gradient(180deg,rgba(var(--background-rgb),0.62)_0%,rgba(var(--background-rgb),0.8)_42%,rgba(var(--background-rgb),0.98)_78%)]" aria-hidden />
      ...
      <div className="mx-auto flex min-h-[min(720px,calc(100svh-4rem))] w-full max-w-[1560px] items-end px-[clamp(1rem,5vw,2rem)] pb-[clamp(2.75rem,8vh,4rem)] pt-20 sm:min-h-[680px] sm:px-8 sm:pb-20 sm:pt-28 md:min-h-[720px] lg:min-h-[790px] lg:items-center lg:px-12 lg:py-28 xl:min-h-[840px] 2xl:min-h-[880px] 2xl:px-16">
