...
type Row={project:{id:string;title:string;slug:string;year:number|null;status:string;isFeatured:boolean;revision:number;thumbnailUrl:string|null;thumbnailAlt:string|null;posterUrl:string|null};category:{name:string}|null;videoOrientation:"auto"|"portrait"|"landscape"};
    ...
    <table className={styles.table}>
      ...
      <tbody>{initialRows.map(({project,category})=>{
        ...
        return <tr key={project.id}>
          <td data-label="Project Banner"><div className="relative aspect-video w-[128px] overflow-hidden rounded-md border border-white/10 bg-black/35">{imageUrl?<Image src={imageUrl} alt={project.thumbnailAlt||project.title} fill sizes="128px" className="object-cover object-center"/>:<div className="flex h-full items-center justify-center px-2 text-center text-[10px] uppercase tracking-[.14em] text-[var(--text-readable,var(--text-muted))]">No banner</div>}</div></td>
          <td data-label="Details" className={styles.tableTitle}><div>{project.title}</div><div className="text-xs font-normal text-[var(--text-readable,var(--text-muted))]">{category?.name??"Uncategorized"}{project.year?` · ${project.year}`:""}</div></td>
