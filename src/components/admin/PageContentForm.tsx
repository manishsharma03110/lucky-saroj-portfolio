...
export function PageContentForm({ pageKey, content, revision, heroImageAssetId }: { pageKey: PageContentKey; content: Record<string, string>; revision: number; heroImageAssetId?: string | null; }) {
  ...
  return (
    <MediaForm id={`${pageKey}-content`} action={action} className={`${styles.sectionStack} scroll-mt-24`}>
      ...
      <FormCard title={config.label}><div className="grid grid-cols-1 gap-5 lg:grid-cols-2">{fields.map((field) => {
        ...
        if (field.kind === "contact-options") return <div key={field.key} className="lg:col-span-2"><ContactOptionsEditor name={field.key} defaultValue={content[field.key] ?? field.defaultValue} /></div>;
