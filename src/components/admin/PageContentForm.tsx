...
export function PageContentForm({ pageKey, content, revision }: { pageKey: PageContentKey; content: Record<string, string>; revision: number }) {
          ...
          {config.fields.map((field) => (
            <div key={field.key} className={field.kind === "textarea" ? "lg:col-span-2" : undefined}>
              ...
              {field.kind === "textarea" ? (
                ...
                <Input id={`${pageKey}-${field.key}`} name={field.key} type={field.kind === "url" ? "text" : "text"} maxLength={field.maxLength ?? 300} defaultValue={content[field.key] ?? field.defaultValue} />
              )}
            </div>
          ))}
        </div>
