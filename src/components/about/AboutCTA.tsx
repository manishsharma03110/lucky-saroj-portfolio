    ...
    <section className="relative overflow-hidden bg-[var(--background-primary)] py-12 sm:py-14 lg:py-14">
      ...
      <div className="relative mx-auto w-full max-w-[1480px] px-5 sm:px-8 lg:px-12 2xl:px-16">
        <div className="grid items-end gap-8 border-l border-[var(--accent-primary)]/55 pl-6 sm:pl-9 lg:grid-cols-[minmax(0,1fr)_auto] lg:gap-16">
          <div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--accent-primary)]">{eyebrow}</p><h2 className="mt-5 max-w-3xl font-display text-[clamp(2rem,3vw,3.4rem)] font-semibold leading-[1] tracking-[-0.045em] text-[var(--text-primary)]">{heading}</h2><p className="mt-5 max-w-xl text-base leading-7 text-[var(--text-secondary)]">{description}</p></div>
          <div className="flex flex-col gap-3 sm:flex-row lg:justify-end"><Link href={primaryUrl} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-[var(--accent-primary)] px-6 py-3 text-sm font-semibold text-[var(--background-primary)] transition-colors hover:bg-[var(--accent-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-4 focus-visible:ring-offset-[var(--background-primary)]">{primaryLabel} <ArrowUpRight size={16} aria-hidden /></Link><Link href={secondaryUrl} className="inline-flex min-h-12 items-center justify-center rounded-md border border-white/20 px-6 py-3 text-sm font-semibold text-[var(--text-primary)] transition-colors hover:border-[var(--accent-primary)] hover:text-[var(--accent-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)]">{secondaryLabel}</Link></div>
        </div>
      </div>
    </section>
  );
