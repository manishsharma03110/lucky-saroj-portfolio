...
export default async function AdminPortfolioPage(){
  ...
  const [rows,page,seo]=await Promise.all([db.select({project:schema.portfolioProjects,category:schema.portfolioCategories}).from(schema.portfolioProjects).leftJoin(schema.portfolioCategories,eq(schema.portfolioProjects.categoryId,schema.portfolioCategories.id)).orderBy(schema.portfolioProjects.displayOrder),getPageContent("portfolio"),getPageSeo("portfolio")]);
