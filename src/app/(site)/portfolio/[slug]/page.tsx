...
import { projectCreativeWorkJsonLd, projectMediaVideoJsonLd } from "@/lib/structured-data";
...
async function socialSeo(slug: string) {
  const result = await db.execute<SocialSeo>(sql`
    SELECT
      og_title AS "ogTitle",
...
async function projectVideoFields(slug: string) {
  const result = await db.execute<ProjectVideoFields>(sql`
    SELECT
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const [data, settings, social] = await Promise.all([getProjectBySlug(slug), getSiteSettings(), socialSeo(slug)]);
  if (!data || data.project.status !== "published") return { robots: { index: false, follow: false } };

  const project = data.project;
  const title = project.seoTitle || project.title;
  const description = project.seoDescription || project.description || `Watch ${project.title}, a video editing project by Lucky Saroj.`;
  const ogTitle = social?.ogTitle || title;
  const ogDescription = social?.ogDescription || description;
  const ogImage = social?.ogImageUrl || project.thumbnailUrl || project.posterUrl || settings?.ogImageUrl || undefined;
  const twitterTitle = social?.twitterTitle || ogTitle;
  const twitterDescription = social?.twitterDescription || ogDescription;
  const twitterImage = social?.twitterImageUrl || ogImage;
  const keywords = [project.title, data.category?.name, project.clientName, project.year ? String(project.year) : null, "video editing", "Lucky Saroj"].filter((value): value is string => Boolean(value));
  ...
  return {
    ...
    openGraph: {
      ...
      ...(ogImage ? { images: [absoluteSiteUrl(ogImage)] } : {}),
    },
    twitter: {
      card: twitterImage ? "summary_large_image" : "summary",
      title: twitterTitle,
      description: twitterDescription,
      ...(twitterImage ? { images: [absoluteSiteUrl(twitterImage)] } : {}),
    },
  };
}
...
export default async function ProjectDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const [data, portfolioPage, videoFields] = await Promise.all([
    getProjectBySlug(slug),
    getPageContent("portfolio"),
    projectVideoFields(slug),
  ]);
  if (!data || data.project.status !== "published" || !videoFields) notFound();

  const project = { ...data.project, ...videoFields };
  const { category, tools, media } = data;
  const rootVideos = new Set(
    [project.hostedVideoUrl?.trim(), project.videoUrl?.trim()].filter((value): value is string => Boolean(value))
  );
  const schemaMedia = media.filter((item) => item.type !== "video" || !rootVideos.has(item.url.trim()));
  ...
  const preferredVideoUrl = project.hostedVideoUrl?.trim() || project.videoUrl?.trim() || null;
    ...
    <PageMotionBoundary className="bg-[var(--background-primary)] text-[var(--text-primary)]">
      <JsonLd
        data={projectCreativeWorkJsonLd({
          ...
          thumbnailUrl: project.thumbnailUrl ?? project.posterUrl,
      ...
      <ProjectNavigation
        previous={prev}
        next={next}
        ariaLabel={copy.detailNavigationAriaLabel}
        previousLabel={copy.detailPreviousLabel}
        nextLabel={copy.detailNextLabel}
      />
      <ProjectCTA
