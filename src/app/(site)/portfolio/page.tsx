            ...
            <>
              ...
              {collection.length > 0 && (
                <div className="pt-16 sm:pt-20 lg:pt-24">
                  <div className="flex items-end justify-between gap-6 border-b border-white/10 pb-6"><div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--accent-primary)]">{copy.collectionEyebrow}</p><h2 className="mt-3 font-display text-3xl font-semibold tracking-[-0.04em] text-[var(--text-primary)] sm:text-4xl">{copy.collectionHeading}</h2></div><p className="hidden text-sm text-[var(--text-muted)] sm:block">{collection.length} {collection.length === 1 ? "project" : "projects"}</p></div>
                  <div className="grid grid-cols-1 gap-x-8 gap-y-10 pt-9 sm:gap-y-12 sm:pt-10 lg:grid-cols-12 lg:gap-x-10 2xl:gap-x-12 2xl:gap-y-14">
              ...
              {hasCaseStudy && (
                <aside className="mt-16 border-y border-white/10 bg-[var(--surface-primary)] px-6 py-9 sm:mt-20 sm:px-8 sm:py-11 lg:mt-24 lg:grid lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)_auto] lg:items-end lg:gap-12 lg:px-10">
                  <div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--accent-primary)]">{copy.caseStudyEyebrow}</p><p className="mt-3 text-sm text-[var(--text-muted)]">{[featured.category?.name, featured.project.clientName, featured.project.year].filter(Boolean).join(" · ")}</p></div>
