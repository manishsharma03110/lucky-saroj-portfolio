...
export default async function PortfolioPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  const [categories, projects, page, seo] = await Promise.all([getCategories(), getPublishedProjects({ categorySlug: category }), getPageContent("portfolio"), getPageSeo("portfolio")]);
  ...
  return (
    <PageMotionBoundary className="bg-[var(--background-primary)] text-[var(--text-primary)]">
      ...
      <PortfolioHero projectCount={projects.length} categoryName={activeCategory?.name} eyebrow={copy.heroEyebrow} heading={seo.pageH1 || copy.heroHeading} description={copy.heroDescription} />
        ...
        <div className="mx-auto w-full max-w-[1480px] px-5 sm:px-8 lg:px-12 2xl:px-16">
          {featured ? <>
            ...
            {collection.length > 0 && <div className="pt-16 sm:pt-20 lg:pt-24">
              <div className="flex items-end justify-between gap-6 border-b border-white/10 pb-8"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--accent-primary)]">{copy.collectionEyebrow}</p><h2 className="mt-4 max-w-[14ch] font-display text-[clamp(2.25rem,3.2vw,3.5rem)] font-semibold leading-[1.02] tracking-[-0.045em] text-[var(--text-primary)]">{seo.pageH2 || copy.collectionHeading}</h2></div><p className="hidden text-sm text-[var(--text-muted)] sm:block">{collection.length} {collection.length === 1 ? copy.collectionProjectSingular : copy.collectionProjectPlural}</p></div>
