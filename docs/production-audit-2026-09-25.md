...
- Axe results and lab performance diagnostics are attached to the GitHub Actions report. Critical accessibility results gate CI; other findings must be examined separately and are not automatically counted as passed.
...
| H03 | Media | Low / P2 | Desktop portfolio images have grayscale applied until hover; hero requested quality 40. | Preserve original media color, use hero quality 75 and responsive srcset. Removed grayscale filter. | Source inspected; preview review; no source image replaced. |
...
| F01 | Contact attachment | Medium / P1 | UI/API allowed 10 MB through a Vercel function limited to 4.5 MB requests. | Consistent 4 MB cap leaves multipart overhead; client validation and early server size check. | Source and API/UI regressions. CMS direct-to-Blob video uploads retain their existing larger limits. |
