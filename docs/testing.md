...
Flexible-banner coverage uses non-standard portrait (210×330) and landscape (350×190) fixtures, saves without size/manual orientation, preserves the banner on unrelated edits, saves with no banner, and checks a single center-cover artwork layer on Home, Portfolio, Related Projects and Project Detail. These CI image uploads mock only the Blob transport response; project persistence and asset attachment use the real disposable database. Production Blob upload is a separate authorized release verification.
